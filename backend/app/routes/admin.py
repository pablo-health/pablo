...
def perform_export_action(
    ...
    # If approved, export to Braintrust (if configured)
    if action == "approve":
        settings = get_settings()

        if not settings.is_braintrust_enabled:
            logger.warning(
                f"Session {session_id} approved but Braintrust not configured - skipping export"
            )
        else:
            try:
                # Fetch full session object
                session = TherapySession.from_dict(session_data)

                # Export to Braintrust
                braintrust_service = BraintrustExportService(
                    api_key=settings.braintrust_api_key.get_secret_value(),  # type: ignore[union-attr]
                    project_name=settings.braintrust_project_name,
                )
                record_id = braintrust_service.export_session(session)

                # Update session with export timestamp
                session_ref.update(
                    {
                ...
                logger.info(f"Session {session_id} exported to Braintrust: {record_id}")
            except Exception as e:
                # Log error but don't fail approval
                logger.error(f"Failed to export session {session_id} to Braintrust: {e}")
                # Session remains "approved" but not "exported"
