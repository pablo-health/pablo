...
def validate_rating_feedback(
    rating: int,
    reason: str | None,
    sections: list[SOAPSection] | None,
) -> None:
    """Validate feedback is provided when rating < threshold."""
    settings = get_settings()

    if rating >= settings.rating_feedback_required_below:
        return

    has_reason = reason and reason.strip()
    has_sections = sections and len(sections) > 0
...
class SessionService:
    ...
    def finalize_session(
        ...
        validate_rating_feedback(
            ...
            request.quality_rating_reason,
            request.quality_rating_sections,
        )
