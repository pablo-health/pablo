...
* the box mapping — 1a is the member id, 21 the diagnoses, 24A-J the
  service lines with lettered pointers, 25 the masked tax id, 26 the
...
class TestBoxMapping:
    def test_identity_boxes(self) -> None:
        fields = cms1500_fields(fixture_claim(), now=FIXTURE_NOW)
    ...
    def test_billing_boxes(self) -> None:
        fields = cms1500_fields(fixture_claim(), now=FIXTURE_NOW)
        assert fields.box_25_tax_id == "XX-XXX9714"
    ...
    def test_ssn_tax_id_is_masked_in_its_own_shape(self) -> None:
        ssn = claim(billing_snapshot=billing_snapshot(tax_id_type="ssn", tax_id_last4="1234"))
        assert cms1500_fields(ssn, now=FIXTURE_NOW).box_25_tax_id == "XXX-XX-1234"

    def test_a_pointer_past_the_diagnosis_list_prints_nothing(self) -> None:
        odd = claim(lines=[line(dx_pointers=[1, 3])])
        assert cms1500_fields(odd, now=FIXTURE_NOW).box_24_lines[0].dx_pointer == "A"


class TestRender:
    def test_same_claim_same_clock_same_bytes(self) -> None:
        first = render_cms1500(fixture_claim(), now=FIXTURE_NOW)
        second = render_cms1500(fixture_claim(), now=FIXTURE_NOW)
        assert first == second
        assert first.startswith(b"%PDF")

    def test_matches_the_committed_fixture(self) -> None:
        rendered = render_cms1500(fixture_claim(), now=FIXTURE_NOW)
    ...
    def test_the_clock_only_moves_the_signature_date(self) -> None:
        ...
        text_now = pdf_text(render_cms1500(fixture_claim(), now=FIXTURE_NOW))
        text_later = pdf_text(render_cms1500(fixture_claim(), now=later))
    ...
    def test_every_box_value_is_on_the_page(self) -> None:
        text = pdf_text(render_cms1500(fixture_claim(), now=FIXTURE_NOW))
        for expected in (
    ...
    def test_only_six_service_rows_are_printed(self) -> None:
        ...
        text = pdf_text(render_cms1500(crowded, now=FIXTURE_NOW))
...
def _regenerate_fixture() -> None:
    ...
    FIXTURE_PATH.write_bytes(render_cms1500(fixture_claim(), now=FIXTURE_NOW))
