...
def artifact_service(
    ...
    Present in this file because the assignment routes now depend on it —
    the detail read carries what was attached — rather than because
    anything here exercises an upload. Its own behaviour is tested in
    ``test_patient_intake_artifacts_api.py``.
    """
...
class TestTheClinicianReadsWhatWasAnswered:
    ...
    def test_reading_it_is_a_disclosure_and_is_recorded(
        ...
        assert logged[-1].resource_id == assignment["id"]
        # How many answers were disclosed and how many files came with
        # them, and not a word of any of them.
        assert logged[-1].changes == {"count": 4, "artifacts": 0}
