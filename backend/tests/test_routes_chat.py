...
from app.models import ChatConversation, Patient
...
class TestPatientAccessIDOR:
    ...
    def test_get_other_clinicians_conversation_returns_404(
    ...
    def test_patch_other_clinicians_conversation_returns_404(
        self, client: TestClient
    ) -> None:
        repo = InMemoryChatRepository()
        self._override_chat_repo(repo)
    ...
    def test_delete_other_clinicians_conversation_returns_404(
        self, client: TestClient
    ) -> None:
        repo = InMemoryChatRepository()
        self._override_chat_repo(repo)
