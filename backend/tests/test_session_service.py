...
def mock_eval_export_service() -> Mock:
    service = Mock(spec=EvalExportService)
    service.should_queue_for_export.return_value = QueueDecision(
        should_queue=False, reason="Not queued"
    )
    return service
...
def service(
    ...
    return SessionService(session_repo, patient_repo, mock_soap_service, mock_eval_export_service)
...
class TestUploadSession:
    ...
    def test_soap_generation_failure(
        ...
        mock_soap_service.generate_soap_note.side_effect = RuntimeError("LLM error")
...
class TestFinalizeSession:
    ...
    def test_low_rating_requires_feedback(
        self,
        service: SessionService,
        patient: Patient,
        user_id: str,
        ...
        request = FinalizeSessionRequest(quality_rating=2)

        with pytest.raises(RatingFeedbackRequiredError):
            service.finalize_session(session.id, user_id, request)
    ...
    def test_eval_export_queued(
        ...
        mock_eval_export_service.should_queue_for_export.return_value = QueueDecision(
            should_queue=True, reason="Low rating"
        )
        mock_eval_export_service.queue_session_for_export.side_effect = lambda s: s

        request = FinalizeSessionRequest(
            quality_rating=1,
            quality_rating_reason="Poor quality",
        )
        service.finalize_session(session.id, user_id, request)
...
class TestUpdateRating:
    ...
    def test_low_rating_requires_feedback(
        ...
        request = UpdateSessionRatingRequest(quality_rating=1)
...
class TestValidateRatingFeedback:
    def test_high_rating_no_feedback_ok(self) -> None:
        validate_rating_feedback(5, None, None)

    def test_low_rating_no_feedback_raises(self) -> None:
        with pytest.raises(RatingFeedbackRequiredError):
            validate_rating_feedback(1, None, None)

    def test_low_rating_with_reason_ok(self) -> None:
        validate_rating_feedback(1, "Needs work", None)

    def test_low_rating_with_sections_ok(self) -> None:
        validate_rating_feedback(1, None, [SOAPSection.ASSESSMENT])

    def test_low_rating_whitespace_reason_raises(self) -> None:
        with pytest.raises(RatingFeedbackRequiredError):
            validate_rating_feedback(1, "   ", None)

    def test_low_rating_empty_sections_raises(self) -> None:
        with pytest.raises(RatingFeedbackRequiredError):
            validate_rating_feedback(1, None, [])
