...
| **LLM Observability** | Braintrust | 0.4.2 | AI call tracing and monitoring |
...
- Braintrust integration (optional, set BRAINTRUST_API_KEY)
...
| `BRAINTRUST_API_KEY` | string | - | Optional: LLM observability |
...
**Tool:** Braintrust (https://www.braintrust.dev)

**Purpose:**
- Monitor AI model performance and quality
- Track SOAP generation latency and token usage
- Analyze model outputs for continuous improvement
- Debug AI-related issues

**Integration:**
```python
# backend/app/main.py
from braintrust import setup_genai

setup_genai()  # Auto-instruments Google Genai SDK
```

**Features:**
- Automatic trace capture for all Vertex AI calls
- Non-blocking - silently skips if `BRAINTRUST_API_KEY` not set
- Captures:
...
- Dashboard access via Braintrust account

**Metrics Tracked:**
- SOAP generation time per transcript
- Token consumption patterns
- Quality rating correlation with prompt variations
