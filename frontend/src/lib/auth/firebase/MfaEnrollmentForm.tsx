...
/**
 * Firebase implementation of the `/mfa-enrollment` surface — TOTP
 * enrollment via Firebase's `multiFactor` API, with reauthentication and
 * email-verification handling. Rendered through
 * `getAuthSurfaces().MfaEnrollmentForm`. The OIDC provider hosts MFA on
 * Keycloak's own pages, so its surface renders nothing.
 */
...
import { useFirebaseUser } from "@/lib/auth/firebase/client"
